...
\definecolor{boxgreen}{HTML}{E6FFED}
\definecolor{boxred}{HTML}{FFE6E6}
\definecolor{boxblue}{HTML}{E6F2FF}
...
\newmdenv[
	...
	frametitlebackgroundcolor=boxgreen,
]{greenbox}
\newmdenv[
	style=boxstyle,
	backgroundcolor=boxred,
	frametitlebackgroundcolor=boxred,
]{redbox}
\newmdenv[
	style=boxstyle,
	backgroundcolor=boxblue,
	frametitlebackgroundcolor=boxblue,
]{bluebox}
\usepackage{booktabs}
...
Existující modely historického myšlení pracují s konceptem tzv. dobových perspektiv (\emph{historical perspectives}). Projekt Dějepis+ je rámoval otázkou \emph{\enquote{Jak lépe porozumět lidem, kteří žili v minulosti?}} \autocite[10]{Cinatl2021Metodika}. Jak ale, že žákům se porozumět lidem v minulosti daří?
...
\emph{Argumentuj na základě pramene\ldots{} Opravdu to pramen říká? \ldots{} Není to
už jen interpretace?}
...
\section{Plánovaný výzkum}\label{pluxe1novanuxfd-vuxfdzkum-1}

\subsection{Proč by to pro učitele mohlo být užitečné?}\label{proux10d-by-to-pro-uux10ditele-mohlo-buxfdt-uux17eiteux10dnuxe9}
...
\item
  podněty, kde mají žáci tendenci \enquote{přeskakovat} k rychlým závěrům,
\item
...
\subsection{Jak bude výzkum probíhat?}\label{jak-bude-vuxfdzkum-probuxedhat}

\begin{itemize}
\tightlist
\item
  \textbf{Kdo?} 2. stupeň ZŠ a všechny ročníky SŠ
\item
...
\item
  ...
  \item
    Úkoly se zdroji a tvrzeními - vybírají, zda je tvrzení
    oprávněné, a zdůvodňují proč.
  \item
    Dvě složitější situace z \enquote{reálnějšího} kontextu.
  \end{enumerate}
\end{itemize}

Účast žáků je dobrovolná a anonymní.

\subsubsection{Ukázka úkolu}\label{ukuxe1zka-uxfakolu}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ukázka úkolu}]{judovo-overovani-historickeho-mysleni_files/images/030-ukazka-testove-ulohy.png}}
\caption{Ukázka úkolu}
\end{figure}

\subsection{Co jako zapojený učitel dostanu?}\label{co-jako-zapojenuxfd-uux10ditel-dostanu}
...
\subsubsection{K čemu to může být ve výuce?}\label{k-ux10demu-to-mux16fux17ee-buxfdt-ve-vuxfduce}
...
\section{Výsledky výzkumu}\label{vuxfdsledky-vuxfdzkumu-1}
...
Jinak řečeno: žák často správně vidí, že \emph{něco} v prameni je, ale udělá z toho
...
\textbf{Žáci v průměru zvládli méně než polovinu rozhodnutí.} Samotné
...
\subsection{Jak výzkum probíhal?}\label{jak-vuxfdzkum-probuxedhal-1}

Test vyplnilo celkem 590 žáků. Některá vyplnění jsem musel
vyřadit - typicky žáky, kteří celý test proletěli za pár desítek vteřin. Po tomto
vyčištění zůstalo 524 z nich. Test byl anonymní, probíhal online a
trval v průměru 21.6 minut.

Není to reprezentativní vzorek českých žáků. Jsou to zapojené třídy a
zapojení učitelé. Z výsledků nelze vyvozovat závěry, jak na tom \enquote{čeští žáci jsou}.
...
vzal užší otázku: \emph{opravňuje dostupný důkaz právě k tomuto tvrzení?}

\subsection{Kdo byl ve vzorku?}\label{kdo-byl-ve-vzorku}

Ve výsledcích níže pracuji s 524 žáky po vyčištění dat. Vzorek
...
\hline
Stupeň & Ročník & Počet žáků & Podíl\\
\hline
ESO & 4º ESO & 27 & 5.2 \%\\
\hline
ZŠ & 6. ročník & 51 & 9.7 \%\\
...
\begin{tabular}{l|r|r}
\hline
Pohlaví & Počet žáků & Podíl\\
\hline
dívky & 237 & 45.2 \%\\
\hline
chlapci & 233 & 44.5 \%\\
\hline
jiné / neuvedeno & 54 & 10.3 \%\\
\hline
\end{tabular}

\normalsize

\subsection{Výsledky}\label{vuxfdsledky-1}

Žáci s tím měli zřetelné potíže. V průměru správně vyřešili asi
39 \% otázek typu \enquote{lze toto tvrdit na základě pramene?}
Medián byl 38 \%.

\scriptsize

\begin{center}\includegraphics[width=0.8\linewidth]{judovo-overovani-historickeho-mysleni_files/figure-latex/ek_score_distribution-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{judovo-overovani-historickeho-mysleni_files/figure-latex/ek_construct_summary-1} \end{center}
...
14 \%, tedy poměrně málo. Spíše se v tom asi odráží
...
\subsection{Nejzajímavější zjištění: odpověď a důvod se mohou rozejít}\label{nejzajuxedmavux11bjux161uxed-zjiux161tux11bnuxed-odpovux11bux10f-a-dux16fvod-se-mohou-rozejuxedt}
...
\begin{center}\includegraphics[width=0.8\linewidth]{judovo-overovani-historickeho-mysleni_files/figure-latex/ek_pattern_summary-1} \end{center}

\normalsize

Pouze 22.4 \% odpovědí mělo současně správné rozhodnutí i dobrý
důvod. V 19.4 \% odpovědí bylo rozhodnutí špatně, ale důvod
dával z hlediska práce s důkazem smysl. V 17.2 \% odpovědí to
...
\subsection{Jak vypadaly konkrétní úlohy?}\label{jak-vypadaly-konkruxe9tnuxed-uxfalohy}
...
Ve výuce se u podobných textů vyplatí neptat se jen: \emph{Je to pravda?} Lepší
otázka zní: \emph{Co tento zdroj dokládá bezpečně a u čeho bychom potřebovali
...
Didakticky je zajímavé, že nejde jen o hledání \enquote{správnějšího} pramene.
...
\subsection{Co si z výsledků odnést?}\label{co-si-z-vuxfdsledkux16f-odnuxe9st}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Nestačí se ptát jen na odpověď.} U úloh s prameny je velmi užitečné
  chtít po žácích také důvod: \emph{Čím přesně můžeš z pramene podepřít svou odpověď?}
\item
  \textbf{Chybná odpověď nemusí znamenat špatné uvažování.} Někdy je dobré
  hledat, jestli žák selhal v rozhodnutí, nebo v postupu, kterým si
  rozhodnutí zdůvodnil.
\item
  \textbf{Práce s prameny potřebuje učit vyjadřovat a rozumět různým mírám jistoty.} Ve výuce má smysl
  rozlišovat: \emph{pramen dokládá}, \emph{pramen naznačuje}, \emph{je to možné},
  \emph{nevíme}, \emph{z tohoto pramene to říci nemůžeme}. To je slovní zásoba,
  kterou řada žáku nemá, a přitom je pro práci s prameny zásadní.
\item
  \textbf{Současná pilotní verze není nástroj pro hodnocení škol, učitelů ani přesné srovnávání tříd.}
...
\item
  Dejte jim tři možnosti: \emph{pramen dokládá}, \emph{pramen naznačuje}, \emph{z pramene
  to nelze říci}.
\item
  U silného závěru se ptejte: \emph{Jaký další pramen bychom potřebovali, aby
  bylo možné tvrdit tohle s větší jistotou?}
\end{itemize}

\subsection{Co z toho plyne pro další výzkum?}\label{co-z-toho-plyne-pro-dalux161uxed-vuxfdzkum}

Pilot mi ukázal dvě věci. Tenhle typ úloh má smysl. Zároveň je řadu z nich
potřeba přepsat. Při kontrole, jak jednotlivé úlohy fungovaly, vyšlo 19 položek k
přepracování a dalších 17 jako hraničních. U pilotu je to
